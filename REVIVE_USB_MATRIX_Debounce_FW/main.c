...
/** VECTOR REMAPPING ***********************************************/
	...
	void YourLowPriorityISRCode()
	{
		...
		if(INTCONbits.TMR0IF == 1)
		{
			...
			if(!PIN_IN1)	button_state[0] |= 0x01;
				else		button_state[0] &= 0xfe;
			if(!PIN_IN2)	button_state[0] |= 0x02;
				else		button_state[0] &= 0xfd;
			if(!PIN_IN3)	button_state[0] |= 0x04;
				else		button_state[0] &= 0xfb;
			if(!PIN_IN4)	button_state[0] |= 0x08;
				else		button_state[0] &= 0xf7;
			if(!PIN_IN5)	button_state[0] |= 0x10;
				else		button_state[0] &= 0xef;
			if(!PIN_IN6)	button_state[0] |= 0x20;
				else		button_state[0] &= 0xdf;

			//	二列目
			PIN_OUT2 = 0;	PIN_OUT1 = 1;	PIN_OUT3 = 1;	PIN_OUT4 = 1;	PIN_OUT5 = 1;	PIN_OUT6 = 1;

			if(!PIN_IN1)	button_state[1] |= 0x01;
				else		button_state[1] &= 0xfe;
			if(!PIN_IN2)	button_state[1] |= 0x02;
				else		button_state[1] &= 0xfd;
			if(!PIN_IN3)	button_state[1] |= 0x04;
				else		button_state[1] &= 0xfb;
			if(!PIN_IN4)	button_state[1] |= 0x08;
				else		button_state[1] &= 0xf7;
			if(!PIN_IN5)	button_state[1] |= 0x10;
				else		button_state[1] &= 0xef;
			if(!PIN_IN6)	button_state[1] |= 0x20;
				else		button_state[1] &= 0xdf;

			//	三列目
			PIN_OUT3 = 0;	PIN_OUT2 = 1;	PIN_OUT1 = 1;	PIN_OUT4 = 1;	PIN_OUT5 = 1;	PIN_OUT6 = 1;

			if(!PIN_IN1)	button_state[2] |= 0x01;
				else		button_state[2] &= 0xfe;
			if(!PIN_IN2)	button_state[2] |= 0x02;
				else		button_state[2] &= 0xfd;
			if(!PIN_IN3)	button_state[2] |= 0x04;
				else		button_state[2] &= 0xfb;
			if(!PIN_IN4)	button_state[2] |= 0x08;
				else		button_state[2] &= 0xf7;
			if(!PIN_IN5)	button_state[2] |= 0x10;
				else		button_state[2] &= 0xef;
			if(!PIN_IN6)	button_state[2] |= 0x20;
				else		button_state[2] &= 0xdf;

			//	四列目
			PIN_OUT4 = 0;	PIN_OUT3 = 1;	PIN_OUT1 = 1;	PIN_OUT2 = 1;	PIN_OUT5 = 1;	PIN_OUT6 = 1;

			if(!PIN_IN1)	button_state[3] |= 0x01;
				else		button_state[3] &= 0xfe;
			if(!PIN_IN2)	button_state[3] |= 0x02;
				else		button_state[3] &= 0xfd;
			if(!PIN_IN3)	button_state[3] |= 0x04;
				else		button_state[3] &= 0xfb;
			if(!PIN_IN4)	button_state[3] |= 0x08;
				else		button_state[3] &= 0xf7;
			if(!PIN_IN5)	button_state[3] |= 0x10;
				else		button_state[3] &= 0xef;
			if(!PIN_IN6)	button_state[3] |= 0x20;
				else		button_state[3] &= 0xdf;

			//	五列目
			PIN_OUT5 = 0;	PIN_OUT4 = 1;	PIN_OUT1 = 1;	PIN_OUT2 = 1;	PIN_OUT3 = 1;	PIN_OUT6 = 1;

			if(!PIN_IN1)	button_state[4] |= 0x01;
				else		button_state[4] &= 0xfe;
			if(!PIN_IN2)	button_state[4] |= 0x02;
				else		button_state[4] &= 0xfd;
			if(!PIN_IN3)	button_state[4] |= 0x04;
				else		button_state[4] &= 0xfb;
			if(!PIN_IN4)	button_state[4] |= 0x08;
				else		button_state[4] &= 0xf7;
			if(!PIN_IN5)	button_state[4] |= 0x10;
				else		button_state[4] &= 0xef;
			if(!PIN_IN6)	button_state[4] |= 0x20;
				else		button_state[4] &= 0xdf;

			//	六列目
			PIN_OUT6 = 0;	PIN_OUT5 = 1;	PIN_OUT1 = 1;	PIN_OUT2 = 1;	PIN_OUT3 = 1;	PIN_OUT4 = 1;

			if(!PIN_IN1)	button_state[5] |= 0x01;
				else		button_state[5] &= 0xfe;
			if(!PIN_IN2)	button_state[5] |= 0x02;
				else		button_state[5] &= 0xfd;
			if(!PIN_IN3)	button_state[5] |= 0x04;
				else		button_state[5] &= 0xfb;
			if(!PIN_IN4)	button_state[5] |= 0x08;
				else		button_state[5] &= 0xf7;
			if(!PIN_IN5)	button_state[5] |= 0x10;
				else		button_state[5] &= 0xef;
			if(!PIN_IN6)	button_state[5] |= 0x20;
				else		button_state[5] &= 0xdf;

			PIN_OUT6 = 1;
			
			for(fi = 0;fi < NUM_OF_PINS; fi++)
			{
				if(((button_state[(fi/6)] & (0x01 << (fi % 6))) ? 1 : 0))
				{ //ON
...
//			uc_temp = ReadEEPROM_Agree(EEPROM_SAVE_NUM*(fi*NUM_OF_SETTINGS+fj), EEPROM_SAVE_NUM, EEPROM_SAME_COUNT, &eeprom_data[fi][fj]);
	...
	eeprom_smpl_interval = ReadEEPROM(NUM_OF_PINS*NUM_OF_SETTINGS) ;
	eeprom_check_count = ReadEEPROM(NUM_OF_PINS*NUM_OF_SETTINGS+1) ;
    ...
    if(!HIDRxHandleBusy(USBOutHandle))				//Check if data was received from the host.
    {
        switch(ReceivedDataBuffer[0])
        {
            case 0x80:  // EEPROMにデータを設定
				...
				{
					eeprom_data[ReceivedDataBuffer[6]][0] = ReceivedDataBuffer[7];
					eeprom_data[ReceivedDataBuffer[6]][1] = ReceivedDataBuffer[8];
					eeprom_data[ReceivedDataBuffer[6]][2] = ReceivedDataBuffer[9];
					WriteEEPROM(ReceivedDataBuffer[6]*NUM_OF_SETTINGS,eeprom_data[ReceivedDataBuffer[6]][0]);
					WriteEEPROM(ReceivedDataBuffer[6]*NUM_OF_SETTINGS+1,eeprom_data[ReceivedDataBuffer[6]][1]);
					WriteEEPROM(ReceivedDataBuffer[6]*NUM_OF_SETTINGS+2,eeprom_data[ReceivedDataBuffer[6]][2]);
					
					eeprom_smpl_interval = ReceivedDataBuffer[10];
					eeprom_check_count = ReceivedDataBuffer[11];
					WriteEEPROM(NUM_OF_PINS*NUM_OF_SETTINGS,eeprom_smpl_interval);
					WriteEEPROM(NUM_OF_PINS*NUM_OF_SETTINGS+1,eeprom_check_count);
				}
                break;
            case 0x81:  //ボタンの押下状態を返信
				...
				ToSendDataBuffer[1] = button_state[0] & 0x3f;
				ToSendDataBuffer[2] = button_state[1] & 0x3f;
				ToSendDataBuffer[3] = button_state[2] & 0x3f;
				ToSendDataBuffer[4] = button_state[3] & 0x3f;
				ToSendDataBuffer[5] = button_state[4] & 0x3f;
				ToSendDataBuffer[6] = button_state[5] & 0x3f;
            ...
            case 0x38:	//現在の設定状況を返信その2
                {
	                if(!HIDTxHandleBusy(USBInHandle))
	                {
						...
						for(fi = 0;fi < tmp; fi++)
						{
							for(fj = 0;fj < NUM_OF_SETTINGS;fj++)
							{
								ToSendDataBuffer[fi*NUM_OF_SETTINGS+fj+1] = eeprom_data[(fi+tmp)][fj];
							}
						}
						ToSendDataBuffer[tmp*NUM_OF_SETTINGS+1] = eeprom_smpl_interval;
						ToSendDataBuffer[tmp*NUM_OF_SETTINGS+1+1] = eeprom_check_count;
